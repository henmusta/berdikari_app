...
(function (factory) {
	...
	$.extend($.summernote.plugins, {
						...
						'<div class="modal-dialog" role="document">'+
							'<div class="modal-content">'+
								...
								'<div class="modal-footer">'+
									...
									'<button type="button" class="btn btn-sm btn-primary" id="gt-link-approve">Add</button>'+
								'</div>'+
							'</div>'+
						'</div>'+
					'</div>');
		...
		issuu:function(context){
			var self = this;
			var ui = $.summernote.ui;
			var $editor = context.layoutInfo.editor;
			var options = context.options;
			var lang = options.langInfo;
			var addIssuuListener = function(){
				$('body').on('click','#gt-issuu-approve',function(e){
					context.invoke('editor.restoreRange');
					var url_source = $('#gt-issuu-url').val(),
						url_request = encodeURI('https://issuu.com/oembed?url=' + url_source);
						var html 	= '<div data-url="'+ url_source +'" style="width: 100%; height:400px;" class="issuuembed"></div><script type="text/javascript" src="//e.issuu.com/embed.js" async="true" />';
						context.invoke('editor.pasteHTML', html);
						$('.btn-codeview').trigger('click');
						$('.btn-codeview').trigger('click');
						$('#gt-issuu-dialog').modal('hide');
					
				});
			};
			context.memo('button.issuu',function(){
				var btn = ui.button({
					contents:'<i class="fa fa-file-pdf"/> Issuu',
					tooltip:'Issuu',
					click:function(){
						$('#gt-issuu-url').val('');
						context.invoke('editor.saveRange');
						$('#gt-issuu-dialog').modal('show');
					}
				});
				return btn.render();
			});
			...
			this.initialize = function(){
				this.$panel = $('<div class="modal fade" tabindex="-1" role="dialog" id="gt-issuu-dialog">'+
						'<div class="modal-dialog" role="document">'+
							'<div class="modal-content">'+
								'<div class="modal-header">'+
									'<h4 class="modal-title">Add Issuu Embed</h4>'+
									'<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>'+
								'</div>'+
								'<div class="modal-body">'+
									'<div class="form-group note-form-group"><label class="note-form-label">Url Document</label><input type="text" id="gt-issuu-url" class="form-control"/></div>'+
								'</div>'+
								'<div class="modal-footer">'+
									'<button type="button" class="btn btn-sm btn-default" data-dismiss="modal">Close</button>'+
									'<button type="button" class="btn btn-sm btn-primary" id="gt-issuu-approve">Add</button>'+
								'</div>'+
							'</div>'+
						'</div>'+
					'</div>');
	...
	$(document).ready(function(){
		var summernote_options = {
			...
			toolbar: [
				...
				['misc',['fullscreen','codeview','help','pageBreak','tags','issuu']]
			],
			popover: {
				image: [
