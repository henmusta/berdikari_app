...
class Berita_model extends CI_Model {
	...
	public function datatables($request){
		...
		";
		$columns 			= array("p.title","a.fullname","c.title","p.status","p.date_publish","p.post_id");
		$sql_total_data 	= "SELECT COUNT(DISTINCT post_id) AS total FROM posts WHERE module='berita';";
		$select 			= "
		SELECT 
			...
			string_agg(DISTINCT ARRAY_TO_STRING(c.path, ' > '), ',') AS category, 
			a.fullname AS author 
		";
		$from 				= "
		FROM posts p
			LEFT JOIN post_category_relations r ON r.post_id=p.post_id
			LEFT JOIN news_categories c ON c.category_id=r.category_id
			LEFT JOIN authors a ON a.author_id=p.author_id
		";
		$where 				= "WHERE p.module='berita' ";
		$group_by 			= "GROUP BY p.post_id, a.fullname, c.title ";
		$having 			= "";
		$order_by 			= "";
		$limit 				= "";
		...
		$sql_total_filtered = $with . " SELECT COUNT(DISTINCT p.post_id) AS total " . $from . $where . $having . ";";
		$sql_data 			= $with . $select . $from . $where . $group_by . $having . $order_by . $limit . ";";
		...
		foreach($results->result_array() AS $row){
			...
			$row['actions'] 		= trim('
				<div class="btn-group">
					<a href="berita/edit/'. $row['post_id'] .'" class="btn btn-sm btn-outline-primary js-tooltip-enabled" data-toggle="tooltip" title="" data-original-title="Edit">
	...
	public function single($id){
		...
		FROM post_category_relations r 
			INNER JOIN news_categories c ON c.category_id=r.category_id
		WHERE r.post_id=". $id ."
		ORDER BY c.path ASC;
		";
		$result['categories'] = $this->db->query($sql_categories)->result_array();
		return $result;
	}
	...
	public function select2_categories($filter){
		$row_per_page 	= 10;
		...
		if(isset($filter['q']) && !empty($filter['q'])){
			...
			$where = "WHERE title ILIKE '%". $filter['q'] ."%' ";
		}
		...
		$data = $this->db->query($with . $select_data . $from . $where . $order_by. " LIMIT ". $row_per_page ." OFFSET ". $offset .";");
		return array( 
