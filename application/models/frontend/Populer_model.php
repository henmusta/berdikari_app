...
class Populer_model extends MY_Model {
	...
	public function lists($limit = 10 , $offset = 0, $except_id = 0){
		...
		WHERE p.post_id!=". $except_id ." AND p.status='publish' AND p.module='berita' AND p.date_publish <= NOW() 
	...
	public function loadmore($page = 1){
		$row_per_page 	= 8;
		$total_data 	= $this->get_total_populer();
